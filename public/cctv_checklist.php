...
if (defined('SUPABASE_URL') && defined('SUPABASE_ANON_KEY')) {
    ...
    // Filter branches that start with "texol" (case-insensitive)
    $branches = array_values(array_filter($allBranches, function($branch) {
        $branchName = strtolower(trim($branch['name'] ?? ''));
        return stripos($branchName, 'texol') === 0;
    }));
  ...
  $checklists = array_values(array_filter($checklists, function ($checklist) use ($userId) {
    ...
    // Only show texol checklists
    if (($checklist['branch_type'] ?? '') !== 'texol') {
        return false;
    }
...
<head>
    ...
    <title>CCTV Surveillance Checklist (Texol) - THI Support</title>
...
<body class="dashboard-body">
    <div class="d-flex" id="layoutWrapper">
        <?php $activeMenu = 'cctv_checklist_texol';
        ...
        <div class="main-content flex-grow-1 d-flex flex-column">
            ...
            <main class="flex-grow-1 py-4 py-md-5 px-3 px-lg-4 content-area">
                <section class="mb-4">
                    <h1 class="h4 fw-semibold mb-1">CCTV Surveillance Checklist (Texol Stations)</h1>
                    <p class="text-muted small mb-0">Create, view and print daily CCTV surveillance reports for Texol stations.</p>
                </section>
                <section class="mb-4">
        ...
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">New CCTV Checklist (Texol)</h5>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-primary" id="saveCctvBtn">
    ...
    <div class="modal fade" id="cctvModal" tabindex="-1">
        <div class="modal-dialog modal-xl">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">CCTV Surveillance Checklist (Texol)</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
    ...
    <script type="module">
        ...
        function renderStationCameraInputs() {
            ...
            console.log('Branches count:', branchesData ? branchesData.length : 0);
            
            if (!branchesData || branchesData.length === 0) {
                container.innerHTML = '<p class="text-muted small text-center py-3">No Texol stations found. Please add stations in Admin Settings.</p>';
                return;
            }
...
<div class="cctv-printout">
  <table class="header-table">
    ...
    <tr>
      ...
      <td class="form-title-cell">CCTV Surveillance Checklist (Texol)</td>
  ...
  <div class="content">
    <div class="doc-heading">TEXOL ENERGIES LIMITED CCTV SURVEILLANCE CHECKLIST (TEXOL)</div>

    <div class="purpose">Purpose: To monitor and maintain operational efficiency of CCTV system across all Texol stations.</div>
    ...
    <div class="footer-note">Texol Energies Limited CCTV Surveillance Checklist</div>
  </div>
</div>`;
        }
        ...
        form.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                const checklistData = {
                    checklist_date: document.getElementById('checklistDate').value,
                    station_data: JSON.stringify(stationData),
                    station_count: stationData.length,
                    total_cameras: totalCameras,
                    functioning_cameras: functioningCameras,
        ...
        document.getElementById('modalDownloadCctv')?.addEventListener('click', async () => {
            ...
            try {
                ...
                // Save the PDF
                pdf.save(`cctv_checklist_${selectedCctv.checklist_date || 'date'}.pdf`);
